...
fun getOfflineModelConfig(type: Int): OfflineModelConfig? {
    when (type) {
        ...
        7 -> {
            val modelDir = "sherpa-onnx-nemo-fast-conformer-ctc-be-de-en-es-fr-hr-it-pl-ru-uk-20k-int8"
            return OfflineModelConfig(
                nemo = OfflineNemoEncDecCtcModelConfig(
                    model = "$modelDir/model.int8.onnx",
                ),
        ...
        9 -> {
            val modelDir = "sherpa-onnx-nemo-fast-conformer-ctc-en-de-es-fr-14288-int8"
            return OfflineModelConfig(
                nemo = OfflineNemoEncDecCtcModelConfig(
                    model = "$modelDir/model.int8.onnx",
                ),
        ...
        100 -> {
            ...
            return OfflineModelConfig(
                senseVoice = OfflineSenseVoiceModelConfig(
                    model = "$modelDir/model-10-seconds.rknn",  // 👈 RKNN 模型  sherpa-onnx-nemo-fast-conformer-ctc-en-de-es-fr-14288-int8
                ),
                tokens = "$modelDir/tokens.txt",
        ...
        101 -> {
            ...
            return OfflineModelConfig(
                whisper = OfflineWhisperModelConfig(
                    encoder = "$modelDir/whisper_encoder_base_10s.rknn",
                    decoder = "$modelDir/whisper_decoder_base_10s.rknn",
                    // 最简配置 - 不设置其他参数
                ),
                tokens = "$modelDir/tokens.txt",
                modelType = "whisper",
